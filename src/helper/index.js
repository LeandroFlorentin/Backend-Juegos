...
module.exports = {
    traerGeneros: async () => {
        return await axios.get(`http://api.rawg.io/api/genres?key=${apiKey}`)
            .then(datos => datos.data.results.map(dato => ({ nombre: dato.name })))
    },
    traerJuegos: async () => {
        const paginaUno = await axios.get(`http://api.rawg.io/api/games?key=${apiKey}&page_size=40`)
            .then(ele => ele.data.results)
        const paginaDos = await axios.get(`http://api.rawg.io/api/games?key=${apiKey}&page_size=40&page=3`)
            .then(ele => ele.data.results)
        const paginaTres = await axios.get(`http://api.rawg.io/api/games?key=${apiKey}&page_size=20&page=4`)
            .then(ele => ele.data.results)
    ...
    traerJuego: async (id) => {
        const juego = await axios.get(`http://api.rawg.io/api/games/${id}?key=${apiKey}`)
